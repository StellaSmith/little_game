...
engine::assets::Image engine::assets::Image::load(std::span<std::byte const> bytes, std::optional<std::string_view> format)
{
    SPDLOG_INFO("loading image from memory");

    auto *rw_ops = SDL_RWFromConstMem(bytes.data(), bytes.size());
    if (!rw_ops) {
        auto error = engine::sdl::Error::current();
        SPDLOG_ERROR("failed to create SDL_RWops: {}", error.what());
        throw std::move(error);
    }

    // since the format must be null-terminated, it gets converted to a std::string
    SDL_Surface *surface = IMG_LoadTyped_RW(rw_ops, true, format ? std::string(*format).c_str() : nullptr);
    if (!surface) {
